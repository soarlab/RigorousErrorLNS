...
exp1 = exp.subs(r*log(2),a)
e= sympy.E
exp1 = exp1.subs(2**r,e**a) #a=r ln2
...
v = errdeltaxr.subs(r,-d*log(-x)/log(2))
dv = diff(v,x)
k  = simplify(dv-dv.args[1])
k = -k.args[3]

k = k.args[0]/2**x + k.args[1]/2**x + k.args[2].subs(4**x,2**x)
k = expand(k)

dk = diff(k,x)


v = errdeltaxr.subs(r,-d*x)
dv = simplify(diff(v,x))
k = dv.args[2]
k = k.args[0].subs(x*(d+1),x*d) + k.args[1]/2**x + k.args[2].subs(4**x,2**x)



k = k.subs(2**x,a).subs(x*log(2),log(a)).subs(2**(x*(d+1)),a**(d+1)).subs(2**(d*x),a**d)
k = expand(k)

u =k.subs(a**d,1+d/log(a)).simplify()


dk = diff(k,a)
d2k = diff(dk,a).together().args[2]
d2k3 = d2k.args[0]+d2k.args[1]+d2k.args[2]+d2k.args[3]
p1 = plot(k.subs(d,2**-20), xlim=(0.01,0.3), show=False)
p1.show()
