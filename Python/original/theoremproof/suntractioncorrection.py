# -*- coding: utf-8 -*-
"""
Created on Fri Mar 31 11:28:26 2023

@author: thanh
"""

from sympy import *
import sympy as sympy
from sympy.plotting import plot


x = symbols('x')
X = symbols('X')
Y = symbols('Y')
r = symbols('r')
a = symbols('a')
b = symbols('b')
s = symbols('s')

A = symbols('A')
B = symbols('B')
C = symbols('C')
D = symbols('D')



c = symbols('c')
R = symbols('R')
K = symbols('K')

base = 2
step = 1


delta = log(1-2**x,base)

ddelta = diff(delta,x)

realdeltaxr = log(1-2**(x-r),base)

drrealdeltaxr = diff(realdeltaxr,r)

dxrealdeltaxr = diff(realdeltaxr,x)

approxdeltaxr = delta - r*ddelta

errdeltaxr =  approxdeltaxr - realdeltaxr 

errdeltaxr = together(errdeltaxr)

errdeltaxR = errdeltaxr.subs(r,R)

eratio = errdeltaxr/errdeltaxR

up = errdeltaxr.subs(2**x,a)
down = errdeltaxR.subs(2**x,a)
dup = diff(up,a)
ddown = diff(down,a)
k = dup/ddown


#l = lim x->-oo of ratio

dxeratio = together(diff(eratio,x))
# dxeratio = A0 * A1 * A2 * A*3 * A4 * A*5
# Ai = dxeratio.args[i] > 0 for i = 0,1,2,3,5
dxerationum4 = dxeratio.args[4]


er = errdeltaxr.args[2]
der = diff(er,x)
eR = errdeltaxR.args[2]
deR = diff(eR,x)

symnum4 = dxerationum4.subs(er,a).subs(eR,b)


fr =  er/der
fr = fr.subs(2**x,A)

dfr=together(diff(fr,r))
dfr=dfr.subs(2**r,B).subs(r*log(2),log(B))
#dfr = dfr0 * dfr1 * dfr2 * dfr3
#sign drf = sign dfr3

dfr2 = expand(dfr.args[2]) #sign dfr3 = -sign dfr, want to prove dfr<0 -> dfr3>0
dfr2 = factor(dfr2)
dfr = dfr.subs(dfr.args[2],dfr2)
#dfr3 = dfr3i * (X+1) * (2^R-1)  ==> sign dfr3 = sign dfr3i
#dfr3i = dfr3.args[0] * dfr3.args[3] #=0 when X=0


K = dfr.args[4]
H = dfr/K
M = H.args[2].args[0]
H = H.subs(M,simplify(M))
H = H.subs(log((A+B)/B), log(A+B) - log(B))
M = H.args[1].args[0]
H = H.subs(M,expand(M))

K = K.subs(log(-A/B+1), log(B-A)-log(B))
K = expand(K)


dBK = diff(K,B)
d2BK = together(diff(dBK,B))
d2BK2 = simplify(d2BK.args[2])
d2BK = d2BK.subs(d2BK.args[2],d2BK2)




# Theorem 2


Pmax = simplify(eratio.subs(x,-1))
Pinf = (2**-r - 1 + r*log(2))/(2**-R-1+R*log(2))
F = Pmax - Pinf
dF = together(diff(F,r))
dF = dF.subs(2**R,X).subs(R*log(2), log(X))
dF = dF.subs(log(2-1/X), log(2*X-1)-log(X))
dF = dF.subs(2**r, Y)

dF4 = collect(expand(dF.args[4]),Y**2)
dF41 = collect(dF4-dF4.args[1],Y)
dF4 = dF4.subs(dF4-dF4.args[1],dF41)

dF = dF.subs(dF.args[4], dF4)

K = dF/dF4

A = dF4.args[1].args[1]
B = dF4.args[0].args[1]
C = dF4.args[2]+dF4.args[3]

#D = expand(B*B - 4*A*C)

dA = diff(A)
d2A= together(diff(dA)) #>0 ==> A>0

dYdF40 = 2*A*X + B
f = expand(dYdF40)
df = diff(f)
d2f = diff(df)
d3f= simplify(together(diff(d2f)))

dC = diff(C)
d2C = simplify(diff(diff(C))) #>0 => C>0#

dFw = dF.subs(dF4.args[1].args[1],A).subs(dF4.args[0].args[1],B).subs(dF4.args[2]+dF4.args[3],C)





eratiosample = eratio.subs(r,0.2).subs(R,1)


Psup = (2**-r - 1 + r*log(2))/(2**-R-1+R*log(2))

p0 = plot(eratiosample, xlim=(-10,-0.1) ,ylabel = "Eratio")



p1 = plot(diff(eratiosample), xlim=(-10,-1), show=False)
p2 = plot(F.subs(R,1), xlim=(-2,2), ylim = (-0.1,0.1))
#p0 = plot(D.subs(X,0.1), xlim=(0,1),  show=False)
p2.show()
