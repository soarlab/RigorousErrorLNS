...
private lemma deriv_Km (ha : a ∈ Set.Ioo 0 1) : Set.EqOn (deriv (Km a))
    (fun b => (a * a) / (b - a) + a - b / (b - a) + log b + log (1 - a) - log (b - a) + 1) (Set.Ici 1) := by
  unfold Km
...
private lemma deriv2_Km (ha : a ∈ Set.Ioo 0 1) : Set.EqOn (deriv (deriv (Km a)))
...
private lemma deriv_Km_strictAnti (ha : a ∈ Set.Ioo 0 1) : StrictAntiOn (deriv (Km a)) (Set.Ici 1) := by
...
private lemma deriv_Km_neg (ha : a ∈ Set.Ioo 0 1) (hb : 1 < b) : deriv (Km a) b < 0 := by
...
private lemma Km_strictAnti (ha : a ∈ Set.Ioo 0 1) : StrictAntiOn (Km a) (Set.Ici 1) := by
  apply strictAntiOn_of_deriv_neg (convex_Ici 1)
  unfold Km
...
private lemma Km_neg (ha: a ∈ (Set.Ioo 0 1)) (hb : 1 < b) : Km a b < 0 := by
...
private lemma deriv_Gm_b_pos (ha : a ∈ Set.Ioo 0 1) (hb : 1 < b) :
    0 < (deriv (Gm a)) b := by
  unfold Gm
  rw [deriv_div]
...
lemma deriv_Fm_div_pos (ha : a ∈ Set.Ioo 0 1) (hb : 1 < b) (hc : b < c) :
    0 < deriv (fun a ↦ Fm b a / Fm c a) a := by
...
lemma Lemma65_strictMono (hr1 : 0 < r) (hr2 : r < Δ) :
  ...
  have : ContinuousOn (fun i ↦ Qm Δ i r) (Set.Iio 0) := by
    have : ∀ t > 0, DifferentiableOn ℝ (fun i => Em i t) (Set.Iio 0) := by
      intro t ht
      apply DifferentiableOn.mono (@differentiableOn_Em_i t (by linarith))
      exact subset_refl _
