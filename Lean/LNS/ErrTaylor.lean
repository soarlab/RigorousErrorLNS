...
private lemma err_fix_bound (fix : FixedPoint) (hr1 : 0 ≤ r) (hr2 : r ≤ Δ):
...
private lemma err_fix_bound' (fix : FixedPoint) (hr1 : 0 ≤ r) (hr2 : r ≤ Δ):
  ...
  apply le_trans (err_fix_bound fix hr1 hr2)
...
private lemma err_fix_bound_dir (fix : FixedPointDir) (hr1 : 0 ≤ r) (hr2 : r ≤ Δ):
...
theorem Ep_fix_bound (fix : FixedPoint) {i r Δ : ℝ} (hi : i ≤ 0) (hr1 : 0 ≤ r) (hr2 : r < Δ) :
    |Ep_fix fix i r| < (Ep 0 Δ) + (2 + Δ) * fix.ε := by
  apply lt_of_le_of_lt (err_fix_bound' fix hr1 (le_of_lt hr2))
  apply add_lt_add_right
  exact Ep_bound hi hr1 hr2

theorem Ep_fix_bound_dir (fix : FixedPointDir) {i r Δ : ℝ} (hi : i ≤ 0) (hr1 : 0 ≤ r) (hr2 : r < Δ) :
    |Ep_fix fix i r| < (Ep 0 Δ) + (1 + Δ) * fix.ε := by
  apply lt_of_le_of_lt (err_fix_bound_dir fix hr1 (le_of_lt hr2))
  apply add_lt_add_right
  exact Ep_bound hi hr1 hr2

theorem Em_fix_bound (fix : FixedPoint) {i₀ i r Δ : ℝ} (hi₀ : i₀ < 0) (hi : i ≤ i₀) (hr1 : 0 ≤ r) (hr2 : r < Δ) :
  ...
  rw [this]
  exact Em_bound hi₀ hi hr1 hr2

theorem Em_fix_bound_dir (fix : FixedPointDir) {i₀ i r Δ : ℝ} (hi₀ : i₀ < 0) (hi : i ≤ i₀) (hr1 : 0 ≤ r) (hr2 : r < Δ) :
...
theorem ΦTp_err_bound (fix : FixedPoint) {x Δ : ℝ} (hd : 0 < Δ) (hx : x ≤ 0) :
...
theorem ΦTp_err_bound_dir (fix : FixedPointDir) {x Δ : ℝ} (hd : 0 < Δ) (hx : x ≤ 0) :
    |Φp x - ΦTp_fix fix Δ x| < Ep 0 Δ + (1 + Δ) * fix.ε := by
  have eq : Φp x - ΦTp_fix fix Δ x = Ep_fix fix (Iₓ Δ x) (Rₓ Δ x) := by
    unfold ΦTp_fix Ep_fix; rw [i_sub_r_eq_x]; ring_nf
  rw [eq]
  apply Ep_fix_bound_dir fix _ (rx_nonneg hd x) (rx_lt_delta hd x)
  rw [← x_neg_iff_ix_neg hd]; exact hx

/- A first order Taylor approximation error bound for Φ⁻ -/

theorem ΦTm_err_bound (fix : FixedPoint) {x₀ x Δ : ℝ} (hd : 0 < Δ) (hx₀ : x₀ ≤ -Δ) (hx : x ≤ x₀) :
  ...
  rw [eq]
  exact Em_fix_bound fix (ix_lt_zero hd hx₀) (ix_monotone hd hx) (rx_nonneg hd x) (rx_lt_delta hd x)

theorem ΦTm_err_bound_dir (fix : FixedPointDir) {x₀ x Δ : ℝ} (hd : 0 < Δ) (hx₀ : x₀ ≤ -Δ) (hx : x ≤ x₀) :
    |Φm x - ΦTm_fix fix Δ x| < Em (Iₓ Δ x₀) Δ + (1 + Δ) * fix.ε := by
  have eq : Φm x - ΦTm_fix fix Δ x = Em_fix fix (Iₓ Δ x) (Rₓ Δ x) := by
    unfold ΦTm_fix Em_fix; rw [i_sub_r_eq_x]; ring_nf
  rw [eq]
  exact Em_fix_bound_dir fix (ix_lt_zero hd hx₀) (ix_monotone hd hx) (rx_nonneg hd x) (rx_lt_delta hd x)

/- A first order Taylor approximation of Φ⁻ for x ≤ -1 -/

theorem ΦTm_err_bound' (fix : FixedPoint) {x Δ : ℝ}
  ...
  exact ΦTm_err_bound fix (div_one_imp_pos hdn) hx₀ hx

theorem ΦTm_err_bound_dir' (fix : FixedPointDir) {x Δ : ℝ}
    (hdn : ∃ n : ℕ, 1 = n * Δ) (hx : x ≤ -1) :
    |Φm x - ΦTm_fix fix Δ x| < Em (-1) Δ + (1 + Δ) * fix.ε := by
  have hx₀ : -1 ≤ -Δ := by rw [neg_le_neg_iff]; exact div_one_imp_le_one hdn
  rw [← ix_eq_neg_one hdn]
  exact ΦTm_err_bound_dir fix (div_one_imp_pos hdn) hx₀ hx
